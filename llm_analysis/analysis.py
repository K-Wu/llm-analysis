...
class LLMAnalysis:
    ...
    def get_latency_fwd_per_layernorm(
        ...
        compute_latency = (
            input_numel * 5 / (self.get_TFLOPS_per_gpu() * 10**12)
        )
    ...
    def training(
        ...
        if (
            memory_left
            - max(
                estimated_prefetch_memory_per_gpu, loss_bwd_memory_batch_size_1
            )
            < activation_memory_batch_size_1
        ):
            logger.warning(
        ...
        latency_fwd_per_layer_attn_compute = (
            self.get_latency_fwd_per_layer_attn(
        ...
        if activation_recomputation == ActivationRecomputation.FULL:
            ...
            latency_recompute = (
                num_layers_per_gpu * latency_fwd_per_layer_attn_compute
            )
        elif activation_recomputation == ActivationRecomputation.ATTN_COMPUTE:
            latency_recompute = (
                num_layers_per_gpu
